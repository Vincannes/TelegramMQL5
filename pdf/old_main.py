...
from app.domain.valid_session import authorize
...
async def main() -> None:
    ...
    # Define handler inside main
    @client.on(events.NewMessage(chats=int(config["telegram"]["group_id"])))
    async def handler(event):
        if event.message.text:
            ...
            if model:
                ...
                print(f"Signal written to {filename}")
    ...
    try:
        if not session_exists:
            ...
            print("Existing session found, connecting directly...")
